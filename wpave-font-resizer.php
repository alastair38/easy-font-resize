...
function wpavefrsz_output_resizer($position = '') {
	...
	// get default text strings to override as and when necessary
	
	$decreaseText = get_option('wpavefrsz_text_decrease');
	$increaseText = get_option('wpavefrsz_text_increase');
	$resetText = get_option('wpavefrsz_text_reset');
	
	// check locale of given page to output translated button text
	
	if(get_locale() === 'de_DE'):
	$text = "Textgröße ändern";
	$decreaseText = "Textgröße verkleinern";
	$increaseText = "Textgröße erhöhen";
	$resetText = "Textgröße zurücksetzen";
	elseif(get_locale() === 'fr_FR'):
	$text = 'Redimensionner le texte';
	...
	$html .= '<span class="wpavefrsz-minus' . esc_attr($notranslate_class) . ($use_icons === true ? ' dashicons dashicons-minus' : '') . '" tabindex="0" aria-label="' . esc_attr($decreaseText) . '" title="' . esc_attr($decreaseText) . '" role="button">' . wp_kses_post($minus) . '</span>';
	
	$html .= '<span class="wpavefrsz-plus' . esc_attr($notranslate_class) . ($use_icons === true ? ' dashicons dashicons-plus' : '') . '" tabindex="0" aria-label="' . esc_attr($increaseText) . '" title="' . esc_attr($increaseText) . '" role="button">' . wp_kses_post($plus) . '</span>';
	
	$html .= '<span class="wpavefrsz-reset' . esc_attr($notranslate_class) . ($use_icons === true ? ' dashicons dashicons-image-rotate' : '') . '" tabindex="0" aria-label="' . esc_attr($resetText) . '" title="' . esc_attr($resetText) . '" role="button">' . wp_kses_post($equals) . '</span>';
